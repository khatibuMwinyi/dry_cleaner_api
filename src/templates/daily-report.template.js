export const dailyReportTemplate = (reportData, company = {}) => {
...
<head>
<meta charset="UTF-8" />
<title>Daily Financial Report - ${period.date}</title>
...
<div class="page">
  ...
  <div class="header">
    ...
    <div class="period">${period.date}</div>
  ...
  <div class="section">
    ...
    <div class="two-column">
      ...
      <div class="chart-container">
        <div class="chart-title">Revenue by Customer</div>
    ...
    <table>
      ...
      <tbody>
        ${revenueBreakdown.length > 0 ? 
          revenueBreakdown.map(customer => 
            customer.invoices.map(invoice => `
              <tr>
                <td><strong>${customer.customerName}</strong></td>
                <td>${customer.customerPhone || 'N/A'}</td>
                <td class="num">${invoice.invoiceNumber || invoice._id.toString().slice(-6)}</td>
                <td class="num">${formatTime(invoice.paidAt || invoice.createdAt)}</td>
                <td class="num">${formatTZS(invoice.total)}</td>
              </tr>
            `).join('')
          ).join('') : 
          '<tr><td colspan="5" class="no-data">No revenue data available</td></tr>'}
      </tbody>
    </table>
  </div>
    ...
    <table>
      ...
      <tbody>
        ${expenseBreakdown.length > 0 ? 
          ...
          '<tr><td colspan="3" class="no-data">No expense data available</td></tr>'}
      </tbody>
    </table>
  </div>
